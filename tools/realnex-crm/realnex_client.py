...
IMPORTANT: Uses urllib.request, NOT the requests library.
The requests library returns 401 on RealNex endpoints due to header handling differences.

OData pagination: max 50 records per page ($top=50).
$top > 100 returns 400 Bad Request.
"""
...
class RealNexClient:
    ...
    # ── Helpers ──────────────────────────────────────────────────────
    def iter_all_contacts(self, page_size: int = 50):
        """Iterate all contacts, yielding each one."""
        skip = 0
        while True:
            data = self.get_all_contacts(top=page_size, skip=skip)
    ...
    def iter_all_properties(self, page_size: int = 50):
    ...
    def iter_all_companies(self, page_size: int = 50):
        """Iterate all companies, yielding each one."""
        skip = 0
        while True:
            data = self.get_all_companies(top=page_size, skip=skip)
            records = data.get("value", [])
            if not records:
                break
            for r in records:
